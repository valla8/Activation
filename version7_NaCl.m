...
rho_NaCl_A =  rho_NaCl * AvNmbr / sum(Comp_NaCl.*W_ele); % molecules / cm3
...
%adipose(a) water ()
rho_Na_A = rho_NaCl_A * Comp_NaCl(1); % atoms/cm3
rho_Cl_A = rho_NaCl_A * Comp_NaCl(2); % atoms/cm3
...
for i=1:(numel(x)-1)
    ...
    %protón a la lámina.
    S_NaCl = max(0,1000*S_NaCl_F(currentE*1000)); % MeV/(g/cm2)
    
    %Multiplicamospor la densidad al poder de frenado
    S1 = S_NaCl*rho_NaCl; % MeV/cm
    ...
    % Water (full + simplified)
     ...
     Y_23Mg(i) = Y_23Mg(i)  +  rho_Na_A * max(0,Na23_Mg23_F(E(i))) * 1e-24 * dx;
...
act_Mg23 = zeros(numel(calcTimes), numel(x));
...
for i=1:numel(calcTimes)
    % Water
    act_Mg23(i,:) = deltat * landa_Mg23 .* Y_23Mg .* exp(- landa_Mg23 * calcTimes(i));
...
%%
%Definimos las variables
act_Mg23t = zeros(1,360);
TT=[1:360];

for i=1:360


    act_Mg23t(i) =  landa_Mg23 .* sum(Y_23Mg) .* exp(- landa_Mg23 * i);
...
deltat=0.1;      %Inervalo de tiempo de las simulaciones
a=500/deltat;  %Tiempo de irradación del haz (s)
t=200/deltat;  %Tiempo total de la simulación
tt=240/deltat; %Tiempo de recogida de datos total

c=1;
temp_Mg23=zeros(t+1,numel(x));

temp_total2=zeros(t+1,1);
temp_parcMg23=zeros(t+1,1);
...
for i=1:t+1
    ...
    %if i<a
        for j=1:b
            ...
            temp_Mg23(i,:)=temp_Mg23(i,:)+pps*deltat * Y_23Mg.*landa_Mg23.*exp(-landa_Mg23*(deltat*j-deltat*1));
           buff=sum(temp_Mg23(i,:))/pps;
            
        end
    else
            for j=1:a;
                
            temp_Mg23(i,:)=temp_Mg23(i,:)+pps*deltat * Y_23Mg.*landa_Mg23.*exp(-landa_Mg23*(deltat*j-deltat*1+deltat*c));
   ...
   % else;
    ...
    temp_total2(i)=sum(sum(temp_Mg23(i,:)));
    temp_parcMg23(i)=sum(temp_Mg23(i,:));


    
    
end
    temp_total=temp_Mg23;
