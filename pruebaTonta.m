...
E0 = 62; % Energ?a inicial del haz (MeV)
...
N14_O14_F = fit(N14_O14_E,N14_O14_CS,'smoothingspline','SmoothingParam',0.1)
...
pps=1.6667e+06; %proton per second

x = 0:dx:6; % posiciones en cm.
E = nan(size(x));
Et = nan(size(x));
Eb = nan(size(x));
Ea = nan(size(x));
...
for i=1:(numel(x)-1)
    ...
    sigma_C12_C10p = 0.5 * (max(0,C12_C10_F(E1p)) + max(0,C12_C10_F(E2a)));
...
for i=1:numel(calcTimes)
    ...
    plot(x,100*Ddept);
    ylabel('-dE/dx')
    axis([0 17 0 max(100*Ddept)+0.2*max(100*Ddept)]);
    ...
    legend('Total','C11','N13','O15','C10', 'Location', 'northwest');
    set(gca, 'FontSize', 16)   
    axis([0 21 0 max(act_totala(i,:))+0.2*max(act_totala(i,:))]);   
    yyaxis right
    grid on
    plot(x,100*Ddepa);
    ylabel('-dE/dx')
    axis([0 21 0 max(100*Ddepa)+0.2*max(100*Ddepa)]);
    ...
    plot(x,100*Ddepb);
    ylabel('-dE/dx')
    axis([0 11 0 max(100*Ddepb)+0.2*max(100*Ddepb)]);
    ...
    plot(x,100*Ddepp);
    ylabel('-dE/dx')
    axis([0 17 0 max(100*Ddepp)+0.2*max(100*Ddepp)]);
end
...
c=1;
t=3600;
temp_C10t=zeros(t+1,numel(x));
temp_C11t=zeros(t+1,numel(x));
temp_N13t=zeros(t+1,numel(x));
temp_O15t=zeros(t+1,numel(x));
temp_C10p=zeros(t+1,numel(x));
temp_C11p=zeros(t+1,numel(x));
temp_N13p=zeros(t+1,numel(x));
temp_O15p=zeros(t+1,numel(x));
for i=1:t+1;
    b=i;
    if i<a
        for j=1:b;
...
temp_parcC11t=zeros(t+1);
temp_parcO15t=zeros(t+1);
temp_parcC11p=zeros(t+1);
temp_parcO15p=zeros(t+1);
...
for i=1:t+1;
    ...
    if i<a
        ...
        for j=1:b;
            ...
            %Tissue
            int_C10t(i,:)=int_C10t(i,:)+deltat*Y_C10t-deltat*Y_C10t.*exp(-landa_C10*(deltat*j-deltat*1));
            int_C11t(i,:)=int_C11t(i,:)+deltat*Y_C11t-deltat*Y_C11t.*exp(-landa_C11*(deltat*j-deltat*1));
            int_N13t(i,:)=int_N13t(i,:)+deltat*Y_N13t-deltat*Y_N13t.*exp(-landa_N13*(deltat*j-deltat*1));
            int_O15t(i,:)=int_O15t(i,:)+deltat*Y_O15t-deltat*Y_O15t.*exp(-landa_O15*(deltat*j-deltat*1));
            
            %PMMA
            int_C10p(i,:)=int_C10p(i,:)+deltat*Y_C10p-deltat*Y_C10p.*exp(-landa_C10*(deltat*j-deltat*1));
            int_C11p(i,:)=int_C11p(i,:)+deltat*Y_C11p-deltat*Y_C11p.*exp(-landa_C11*(deltat*j-deltat*1));
            int_N13p(i,:)=int_N13p(i,:)+deltat*Y_N13p-deltat*Y_N13p.*exp(-landa_N13*(deltat*j-deltat*1));
            int_O15p(i,:)=int_O15p(i,:)+deltat*Y_O15p-deltat*Y_O15p.*exp(-landa_O15*(deltat*j-deltat*1));
        ...
        for j=1:a;
            ...
            %Tissue
            int_C10t(i,:)=int_C10t(i,:)+deltat*Y_C10t-deltat*Y_C10t.*exp(-landa_C10*(deltat*j-deltat*1+deltat*c));
            int_C11t(i,:)=int_C11t(i,:)+deltat*Y_C11t-deltat*Y_C11t.*exp(-landa_C11*(deltat*j-deltat*1+deltat*c));
            int_N13t(i,:)=int_N13t(i,:)+deltat*Y_N13t-deltat*Y_N13t.*exp(-landa_N13*(deltat*j-deltat*1+deltat*c));
            int_O15t(i,:)=int_O15t(i,:)+deltat*Y_O15t-deltat*Y_O15t.*exp(-landa_O15*(deltat*j-deltat*1+deltat*c));
            
            %PMMA
            int_C10p(i,:)=int_C10p(i,:)+deltat*Y_C10p-deltat*Y_C10p.*exp(-landa_C10*(deltat*j-deltat*1+deltat*c));
            int_C11p(i,:)=int_C11p(i,:)+deltat*Y_C11p-deltat*Y_C11p.*exp(-landa_C11*(deltat*j-deltat*1+deltat*c));
            int_N13p(i,:)=int_N13p(i,:)+deltat*Y_N13p-deltat*Y_N13p.*exp(-landa_N13*(deltat*j-deltat*1+deltat*c));
            int_O15p(i,:)=int_O15p(i,:)+deltat*Y_O15p-deltat*Y_O15p.*exp(-landa_O15*(deltat*j-deltat*1+deltat*c));
           
        end
            c=c+1;
            
            if i>2
                int_C10p(i,:)=int_C10p(i,:)-int_C10p(i-1,:);
                int_C11p(i,:)=int_C11p(i,:)-int_C11p(i-1,:);
                int_N13p(i,:)=int_N13p(i,:)-int_N13p(i-1,:);
                int_O15p(i,:)=int_O15p(i,:)-int_O15p(i-1,:);
            end
      
    end
           
    temp_totalt2(i)=sum(int_C10t(i,:))+sum(int_C11t(i,:))+sum(int_N13t(i,:))+sum(int_O15t(i,:));
    temp_parcC11t(i)=sum(int_C11t(i,:));
    temp_parcO15t(i)=sum(int_O15t(i,:));
    temp_totalp2(i)=sum(temp_C10p(i,:))+sum(int_C11p(i,:))+sum(int_N13p(i,:))+sum(int_O15p(i,:));
    temp_parcC11p(i)=sum(int_C11p(i,:));
    temp_parcO15p(i)=sum(int_O15p(i,:));
    ...
    xlabel('Tiempo (s)')
    ylabel('Beta+ emitters / s ');
    legend('Actividad total','Location', 'northeast');
    set(gca, 'FontSize', 16) 
    ...
    xlabel('Tiempo (s)')
    ylabel('Beta+ emitters / s ');
    legend('Actividad total','O15','C11','Location', 'northeast');
    set(gca, 'FontSize', 16) 
