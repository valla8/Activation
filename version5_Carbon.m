...
for i=1:(numel(x)-1)
    ...
    %protón a la lámina.
    S_c = max(0,1000*S_Carbon_F(currentEc*1000)); % MeV/(g/cm2)
    
    %Multiplicamospor la densidad al poder de frenado
    S1c = (S_c*rho_C); % MeV/cm
    ...
    sigma_C11_mean = 0.5 * (max(0,C12_C11_F(E1c)) + max(0,C12_C11_F(E2c)));
    Y_C12_C11(i) = pps * rho_C_A * sigma_C11_mean * 1e-24 * dx; 
