...
Gd_fraction= 0.1;     %abundancia del Gd
GdAtomicWeight = 157.25; %g/mol
Gd154_Ab = 0.0218;

% Material H (%) C (%) N (%) O (%) P (%) Ca(%) Gd154(%) Gd155(%) Gd156(%)
% Gd157(%) Gd158(%) Gd160(%)
Comp_Gd = [0 0 0 0 0 0 0.0218 0.148 0.2047 0.1565 0.2484 0.2186];
Comp_cartilage = [0.6232 0.05390 0.0103 0.3042  0.0046 0 0 0 0 0 0 0];
Comp_cartilage_Gd = (1-Gd_fraction)*Comp_cartilage + Gd_fraction*Comp_Gd;
Comp_muscle = [0.631 0.0567 0.0125 0.2982 0 0 0 0 0 0 0 0];
Comp_muscle_Gd = (1-Gd_fraction)*Comp_muscle + Gd_fraction*Comp_Gd;
Comp_bone=[0.475402 0.121899 0.0304112 0.282845 0.0343791 0.0531337 0 0 0 0 0 0];
Comp_bone_Gd = (1-Gd_fraction)*Comp_bone + Gd_fraction*Comp_Gd;

%% Stopping powers
S_Gd_F = fit(E_keV_Gd,S_Gd,'smoothingspline','SmoothingParam',0.002);
S_Gd_F.p.coefs(1,:) = [0 0 0 0];
S_Gd_F.p.coefs(end,:) = [0 0 0 0];

%% calcular (sin struggling)
...
W_eletr = [W_ele 0 0 0 0 0 0];

%rho_tissue_A = (1-Gd_fraction)*AvNmbr*rho_tissue/sum(Comp_tissue.*W_eletr);  % atoms/cm3

rho_bone_A = (1-Gd_fraction)*AvNmbr*rho_bone/sum(Comp_bone.*W_eletr);  % atoms/cm3

rho_cartilage_A = (1-Gd_fraction)*AvNmbr*rho_cartilage/sum(Comp_cartilage.*W_eletr);

rho_muscle_A = (1-Gd_fraction)*AvNmbr*rho_muscle/sum(Comp_muscle.*W_eletr);

rho_Gd_A = Gd_fraction * rho_Gd * AvNmbr / GdAtomicWeight;
...
rho_O16_Ab = rho_bone_A * Comp_bone(4) * O16_ab; % atoms/cm3
...
rho_O16_Ac = rho_cartilage_A * Comp_cartilage_Gd(4) * O16_ab;
rho_N14_Ac = rho_cartilage_A * Comp_cartilage_Gd(3) * N14_ab;
rho_C12_Ac = rho_cartilage_A * Comp_cartilage_Gd(2) * C12_ab;
...
%     sigma_PG_O16_6b = 0.5 * (max(0,PG_O16_O16_6_F(E1b)) + max(0,PG_O16_O16_6_F(E2b)));
    sigma_Gd154_Tb154_meanb = 0.5 * (max(0,Gd154_Tb154_F(E1b)) + max(0,Gd154_Tb154_F(E2b))); 
    ...
    Y_Gd154_Tb154b(i) = rho_Gd_A * Gd154_Ab * sigma_Gd154_Tb154_meanb * 1e-24 * dx;
...
%     sigma_PG_N14_2c = 0.5 * (max(0,PG_N14_N14_2_F(E1c)) + max(0,PG_N14_N14_2_F(E2c)));
%     sigma_PG_O16_6c = 0.5 * (max(0,PG_O16_O16_6_F(E1c)) + max(0,PG_O16_O16_6_F(E2c)));
      sigma_Gd154_Tb154_meanc = 0.5 * (max(0,Gd154_Tb154_F(E1c)) + max(0,Gd154_Tb154_F(E2c))); 
    ...
    Y_Gd154_Tb154c(i) = rho_Gd_A * Gd154_Ab * sigma_Gd154_Tb154_meanc * 1e-24 * dx;
...
%     sigma_PG_O16_6m = 0.5 * (max(0,PG_O16_O16_6_F(E1m)) + max(0,PG_O16_O16_6_F(E2m)));
    sigma_Gd154_Tb154_meanm = 0.5 * (max(0,Gd154_Tb154_F(E1m)) + max(0,Gd154_Tb154_F(E2m))); 
    ...
    Y_Gd154_Tb154m(i) = rho_Gd_A * Gd154_Ab * sigma_Gd154_Tb154_meanm * 1e-24 * dx;
...
Y_C10b = Y_C12_C10b;
Y_Sc44b = Y_Ca44_Sc44;
plot(x,Y_Gd154_Tb154b,'r.'); hold on
plot(x,Y_C11b,'k'); hold on
plot(x,Y_N13b,'c')
plot(x,Y_O15b,'m')
plot(x,Y_C10b,'y');
plot(x,Y_Sc44b,'g');
legend('Gd154-Tb154','C11','N13','O15','C10','Sc44','Location', 'northwest');
[f,g]=min(Ddepb);
...
Y_O15c = Y_O16_O15c;
Y_C10c = Y_C12_C10c;
plot(x,Y_Gd154_Tb154c,'r.'); hold on
plot(x,Y_C11c,'k'); hold on
plot(x,Y_N13c,'c')
plot(x,Y_O15c,'m')
plot(x,Y_C10c,'y');
legend('Gd154-Tb154','C11','N13','O15','C10','Location', 'northwest');
[f,g]=min(Ddepc);
...
Y_O15m = Y_O16_O15m;
Y_C10m = Y_C12_C10m;
plot(x,Y_Gd154_Tb154m,'r.'); hold on
...
act_C11b_Gd = zeros(numel(calcTimes), numel(x)); %bone
act_C10b_Gd = zeros(numel(calcTimes), numel(x));
act_N13b_Gd = zeros(numel(calcTimes), numel(x));
act_O15b_Gd = zeros(numel(calcTimes), numel(x));
act_Sc44b_Gd = zeros(numel(calcTimes), numel(x));
act_Gd154b = zeros(numel(calcTimes), numel(x));
...
act_C11c_Gd = zeros(numel(calcTimes), numel(x)); %cartilage
act_C10c_Gd = zeros(numel(calcTimes), numel(x));
act_N13c_Gd = zeros(numel(calcTimes), numel(x));
act_O15c_Gd= zeros(numel(calcTimes), numel(x));
act_Gd154c = zeros(numel(calcTimes), numel(x));

act_C11m_Gd = zeros(numel(calcTimes), numel(x)); %muscle
act_C10m_Gd = zeros(numel(calcTimes), numel(x));
act_N13m_Gd = zeros(numel(calcTimes), numel(x));
act_O15m_Gd = zeros(numel(calcTimes), numel(x));
act_Gd154m = zeros(numel(calcTimes), numel(x));
...
%     act_O15a(i,:) = deltat * landa_O15 .* Y_O15a .* exp(- landa_O15 * calcTimes(i)); 
     ...
     % Bone
    act_C11b_Gd(i,:) = deltat * landa_C11 .* Y_C11b .* exp(- landa_C11 * calcTimes(i));
    act_C10b_Gd(i,:) = deltat * landa_C10 .* Y_C10b .* exp(- landa_C10 * calcTimes(i));    
    act_N13b_Gd(i,:) = deltat * landa_N13 .* Y_N13b .* exp(- landa_N13 * calcTimes(i));
    act_O15b_Gd(i,:) = deltat * landa_O15 .* Y_O15b .* exp(- landa_O15 * calcTimes(i)); 
    act_Sc44b_Gd(i,:) = deltat * landa_Sc44 .* Y_Sc44b .* exp(- landa_Sc44 * calcTimes(i));
    act_Gd154b(i,:) = deltat * landa_Tb154 .* Y_Gd154_Tb154b.*exp(-landa_Tb154 * calcTimes(i));
     % PMMA
...
%     act_O15p(i,:) = deltat * landa_O15 .* Y_O15p .* exp(- landa_O15 * calcTimes(i)); 
      ...
      % Cartilage
    act_C11c_Gd(i,:) = deltat * landa_C11 .* Y_C11c .* exp(- landa_C11 * calcTimes(i));
    act_C10c_Gd(i,:) = deltat * landa_C10 .* Y_C10c .* exp(- landa_C10 * calcTimes(i));    
    act_N13c_Gd(i,:) = deltat * landa_N13 .* Y_N13c .* exp(- landa_N13 * calcTimes(i));
    act_O15c_Gd(i,:) = deltat * landa_O15 .* Y_O15c .* exp(- landa_O15 * calcTimes(i));
    act_Gd154c(i,:) = deltat * landa_Tb154 .* Y_Gd154_Tb154c.*exp(-landa_Tb154 * calcTimes(i));

      % Muscle
    act_C11m_Gd(i,:) = deltat * landa_C11 .* Y_C11m .* exp(- landa_C11 * calcTimes(i));
    act_C10m_Gd(i,:) = deltat * landa_C10 .* Y_C10m .* exp(- landa_C10 * calcTimes(i));    
    act_N13m_Gd(i,:) = deltat * landa_N13 .* Y_N13m .* exp(- landa_N13 * calcTimes(i));
    act_O15m_Gd(i,:) = deltat * landa_O15 .* Y_O15m .* exp(- landa_O15 * calcTimes(i)); 
    act_Gd154m(i,:) = deltat * landa_Tb154 .* Y_Gd154_Tb154m.*exp(-landa_Tb154 * calcTimes(i));
...
act_totalb = (1/(1-Gd_fraction))*(act_C11b_Gd + act_C10b_Gd + act_N13b_Gd + act_O15b_Gd + act_Sc44b_Gd);
act_totalb_Gd = act_C11b_Gd + act_C10b_Gd + act_N13b_Gd + act_O15b_Gd + act_Sc44b_Gd + act_Gd154b;
% act_totalp = (act_C11p + act_C10p + act_N13p + act_O15p);
act_totalc = (1/(1-Gd_fraction))*(act_C11c_Gd + act_C10c_Gd + act_N13c_Gd + act_O15c_Gd);
act_totalc_Gd = (act_C11c_Gd + act_C10c_Gd + act_N13c_Gd + act_O15c_Gd + act_Gd154c);
act_totalm = (1/(1-Gd_fraction))*(act_C11m_Gd + act_C10m_Gd + act_N13m_Gd + act_O15m_Gd);
act_totalm_Gd = (act_C11m_Gd + act_C10m_Gd + act_N13m_Gd + act_O15m_Gd + act_Gd154m);
...
%     axis([0 (ceil(g*dx)) 0 max(100*Ddepa)+0.2*max(100*Ddepa)]);
    ...
    hold on
    plot(x,act_totalb_Gd(i,:),'m')
%     plot(x, act_C11b_Gd(i,:),'r')
%     plot(x, act_N13b_Gd(i,:),'y')
%     plot(x, act_O15b_Gd(i,:),'c')
%     plot(x, act_C10b_Gd(i,:),'g')
    title(sprintf('Activity at t=%i s ',calcTimes(i)));
    xlabel('Depth (cm)')
    ylabel('Beta+ emitters / s ');
    legend('Total','Total+Gd''C11','N13','O15','C10', 'Location', 'northwest');
    set(gca, 'FontSize', 16) 
...
%     
    ...
    hold on
    plot(x,act_totalc_Gd(i,:),'m')
%     plot(x, act_C11c_Gd(i,:),'r')
%     plot(x, act_N13c_Gd(i,:),'y')
%     plot(x, act_O15c_Gd(i,:),'c')
%     plot(x, act_C10c_Gd(i,:),'g')
    title(sprintf('Activity at t=%i s',calcTimes(i)));
    xlabel('Depth (cm)')
    ylabel('Beta+ emitters / s ');
    legend('Total','total+Gd','C11','N13','O15','C10', 'Location', 'northwest');
...
%     axis([0 5 0 max(act_totalc(i,:))+0.2*max(act_totalc(i,:))]);   
    ...
    hold on
    plot(x,act_totalm_Gd(i,:),'m')
%     plot(x, act_C11m_Gd(i,:),'r')
%     plot(x, act_N13m_Gd(i,:),'y')
%     plot(x, act_O15m_Gd(i,:),'c')
%     plot(x, act_C10m_Gd(i,:),'g')
    title(sprintf('Activity at t=%i s',calcTimes(i)));
    xlabel('Depth (cm)')
    ylabel('Beta+ emitters / s ');
    legend('Total','total+Gd','C11','N13','O15','C10', 'Location', 'northwest');
...
temp_Gd154c=zeros(t+1,numel(x));temp_Gd154m=zeros(t+1,numel(x));
temp_Gd154b=zeros(t+1,numel(x));
...
temp_Gd154_totalb2=zeros(t+1);
temp_Gd154_totalc2=zeros(t+1);
temp_Gd154_totalm2=zeros(t+1);
...
%             temp_O15a(i,:)=temp_O15a(i,:)+deltat * Y_O15a.*landa_O15.*exp(-landa_O15*(deltat*j-deltat*1));
            ...
            temp_Gd154b(i,:)=temp_Gd154b(i,:)+deltat * Y_Gd154_Tb154b.*landa_Tb154.*exp(-landa_Tb154*(deltat*j-deltat*1));
            ...
            temp_Gd154c(i,:)=temp_Gd154c(i,:)+deltat * Y_Gd154_Tb154c.*landa_Tb154.*exp(-landa_Tb154*(deltat*j-deltat*1));
            ...
            temp_Gd154m(i,:)=temp_Gd154m(i,:)+deltat * Y_Gd154_Tb154m.*landa_Tb154.*exp(-landa_Tb154*(deltat*j-deltat*1));
...
%             temp_O15a(i,:)=temp_O15a(i,:)+deltat *Y_O15a.*landa_O15.*exp(-landa_O15*(deltat*j-deltat*1+deltat*c));
            ...
            temp_Gd154b(i,:)=temp_Gd154b(i,:)+deltat *Y_Gd154_Tb154b.*exp(-landa_Tb154*(deltat*j+deltat*1+deltat*c));
            ...
            temp_Gd154c(i,:)=temp_Gd154c(i,:)+deltat * Y_Gd154_Tb154c.*landa_Tb154.*exp(-landa_Tb154*(deltat*j-deltat*1+deltat*c));
            ...
            temp_Gd154m(i,:)=temp_Gd154m(i,:)+deltat * Y_Gd154_Tb154m.*landa_Tb154.*exp(-landa_Tb154*(deltat*j-deltat*1+deltat*c));
...
%     temp_parcO15a(i)=sum(temp_O15a(i,:));
    ...
    temp_totalb2(i)=(1/(1-Gd_fraction))*(sum(temp_C10b(i,:))+sum(temp_C11b(i,:))+sum(temp_N13b(i,:))+sum(temp_O15b(i,:)));
    temp_parcC11b(i)=(1/(1-Gd_fraction))*sum(temp_C11b(i,:));
    temp_parcN13b(i)=(1/(1-Gd_fraction))*sum(temp_N13b(i,:));
    temp_parcC10b(i)=(1/(1-Gd_fraction))*sum(temp_C10b(i,:));
    temp_parcO15b(i)=(1/(1-Gd_fraction))*sum(temp_O15b(i,:));
    temp_Gd154_totalb2(i)=sum(temp_Gd154b(i,:))+sum(temp_C10b(i,:))+sum(temp_C11b(i,:))+sum(temp_N13b(i,:))+sum(temp_O15b(i,:));

    temp_totalc2(i)=(1/(1-Gd_fraction))*(sum(temp_C10c(i,:))+sum(temp_C11c(i,:))+sum(temp_N13c(i,:))+sum(temp_O15c(i,:)));
    temp_parcC11c(i)=(1/(1-Gd_fraction))*sum(temp_C11c(i,:));
    temp_parcN13c(i)=(1/(1-Gd_fraction))*sum(temp_N13c(i,:));
    temp_parcC10c(i)=(1/(1-Gd_fraction))*sum(temp_C10c(i,:));
    temp_parcO15c(i)=(1/(1-Gd_fraction))*sum(temp_O15c(i,:));
    temp_Gd154_totalc2(i)=sum(temp_Gd154c(i,:))+sum(temp_C10c(i,:))+sum(temp_C11c(i,:))+sum(temp_N13c(i,:))+sum(temp_O15c(i,:));
    
    temp_totalm2(i)=(1/(1-Gd_fraction))*(sum(temp_C10m(i,:))+sum(temp_C11m(i,:))+sum(temp_N13m(i,:))+sum(temp_O15m(i,:)));
    temp_parcC11m(i)=(1/(1-Gd_fraction))*sum(temp_C11m(i,:));
    temp_parcN13m(i)=(1/(1-Gd_fraction))*sum(temp_N13m(i,:));
    temp_parcC10m(i)=(1/(1-Gd_fraction))*sum(temp_C10m(i,:));
    temp_parcO15m(i)=(1/(1-Gd_fraction))*sum(temp_O15m(i,:));
    temp_Gd154_totalm2(i)=sum(temp_Gd154m(i,:))+sum(temp_C10m(i,:))+sum(temp_C11m(i,:))+sum(temp_N13m(i,:))+sum(temp_O15m(i,:));
...
%     temp_totala=temp_C10a+temp_C11a+temp_N13a+temp_O15a;
    temp_totalb=(1/(1-Gd_fraction)).*(temp_C10b+temp_C11b+temp_N13b+temp_O15b);
    temp_totalc=(1/(1-Gd_fraction)).*(temp_C10c+temp_C11c+temp_N13c+temp_O15c);
    temp_totalm=(1/(1-Gd_fraction)).*(temp_C10m+temp_C11m+temp_N13m+temp_O15m);
    temp_Gd154_totalb=temp_C10b+temp_C11b+temp_N13b+temp_O15b+temp_Gd154b;
    temp_Gd154_totalm=temp_C10m+temp_C11m+temp_N13m+temp_O15m+temp_Gd154m;
...
%     axis([0 t*deltat 0 (max(temp_totala2(:,1))+0.2*max(temp_totala2(:,1)))]);
    ...
    hold on;
    plot(T*deltat,temp_Gd154_totalb2(:,1));
    ...
    xlabel('Tiempo (s)');
    ylabel('Beta+ emitters /proton/mm ');
    legend('Actividad total','Actividad total+Gd154','O15','C11','N13','C10','Location', 'northeast');
    set(gca, 'FontSize', 16) ;
    ...
    hold on;
    plot(T*deltat,temp_Gd154_totalc2(:,1))
    hold on
    ...
    hold on;
    plot(T*deltat,temp_Gd154_totalm2(:,1))
